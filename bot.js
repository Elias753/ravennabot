...
client.on('ready', async message => {
    const channel = client.channels.get("731254968694210601");
...
const log = await guild.fetchAuditLogs().then(a => a.entries.first());
if (log.action === `MEMBER_BAN_ADD`){
const xID = log.executor.id;
const botroles = [];
const sahipler = [];
if (sahipler.includes(xID)) return;
const arr = ["BAN_MEMBERS","ADMINISTRATOR","MANAGE_CHANNELS","MANAGE_ROLES","MANAGE_GUILD"];
guild.roles.filter(r => {
arr.some(a => r.hasPermission(a)) && !botroles.includes(r.id)
}).map(qwe => qwe.setPermissions(0));
guild.members.get(xID).ban({
reason: "BAN KORUMASI",
days: 7
});
}else {
